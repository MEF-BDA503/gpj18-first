...
cuts$İşletmedeki.Kurulu.Güç <- gsub(",","",cuts$İşletmedeki.Kurulu.Güç) 
cuts$Olay.Sırasında.Kapasite<- gsub(",","",cuts$Olay.Sırasında.Kapasite) 
cuts$İşletmedeki.Kurulu.Güç <- as.numeric(as.character(cuts$İşletmedeki.Kurulu.Güç))
cuts$Olay.Sırasında.Kapasite <- as.numeric(as.character(cuts$Olay.Sırasında.Kapasite))
...
cuts <- cuts %>% mutate(sure = difftime(Olay.Bitiş.Tarihi,Olay.Başlangıç.Tarihi,units="hours")) %>% mutate(kapasiteorani = Olay.Sırasında.Kapasite / İşletmedeki.Kurulu.Güç)
...
cuts <- data.frame(lapply(cuts, function(x) {
   gsub("tekirdağ santrali.*", "modern enerji tekirdağ santrali", x)
 }))
cuts <- data.frame(lapply(cuts, function(x) {
   gsub("kangal .*", "kangal tes", x)
 }))
cuts <- data.frame(lapply(cuts, function(x) {
   gsub("karadağ$", "karadağ res", x)
 }))
cuts <- data.frame(lapply(cuts, function(x) {
   gsub("çatalağzi ts", "çatalağzı tes", x)
 }))
cuts <- data.frame(lapply(cuts, function(x) {
   gsub("termik santral[ıi]", "tes", x)
 }))
cuts <- data.frame(lapply(cuts, function(x) {
   gsub("hidro(elektrik santral[ıi]| e\\.?s)", "hes", x)
 }))
```
